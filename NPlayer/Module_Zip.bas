Attribute VB_Name = "Module_Zip"
Option Explicit
Public ListaCollection As New Collection
...
'Compacta um ou mais arquivos no formato WinZip
    ...
    'Z = addZIP_IncludeDirectoryEntries(azYES)
    Z = addZIP_Include(cArq)
    Z = addZIP_ArchiveName(cArqCompactado)
    Z = addZIP()
End Sub

Sub CompactaLista(cArqCompactado As String, cArq As String)
'Compacta um ou mais arquivos no formato WinZip
    Z = addZIP_SetCompressionLevel(azCOMPRESSION_MAXIMUM)
    Z = addZIP_SaveStructure(azSTRUCTURE_ABSOLUTE) 'StoreFullPathName - azSTRUCTURE_ABSOLUTE
    ...
    Set ListaCollection = New Collection
    Call SUBDIR(cArq)

    For I = 1 To ListaCollection.Count
        DoEvents
        strTmp = CStr(ListaCollection(I))
        
        Z = addZIP_ArchiveName(cArqCompactado)
        Z = addZIP_Include(strTmp)
        Z = addZIP()
    Next
...
Function SUBDIR(STRDIR As String) As String
...
Set fo = fs.GetFolder(STRDIR)

For Each X In fo.SubFolders
    DoEvents
    If X <> Empty Then
        SUBDIR = SUBDIR(STRDIR + "\" + X.Name) + "\*.*"
        If err.Number <> 0 Then
          'MsgBox Err.Description, vbExclamation + vbOKOnly, Err.Source
          'Err.Clear
          Exit For
        End If
        ListaCollection.Add (STRDIR + "\" + X.Name + "\*.*")
    End If
Next
End Function
